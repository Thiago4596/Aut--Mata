...
class LimpezaNavegadores:
    ...
    def delete_files(self, path, pattern):
    ...
    def delete_temp_files(self, user_path):
        ...
        if os.path.exists(temp_path):
            ...
            # Cria arquivo vazio.txt
            with open(os.path.join(temp_path, "vazio.txt"), "w") as f:
                pass
            ...
            for root, dirs, files in os.walk(temp_path, topdown=False):
                for dir_name in dirs:
                    try:
                        ...
                        pass
            # Remove arquivo vazio.txt
            os.remove(os.path.join(temp_path, "vazio.txt"))

    def delete_windows_temp_files(self):
        """Deleta arquivos temporários do Windows."""
        windows_temp_path = os.path.join(os.environ["SystemRoot"], "Temp")
        if os.path.exists(windows_temp_path):
            self.delete_files(windows_temp_path, "*")
            # Cria arquivo vazio.txt
            with open(os.path.join(windows_temp_path, "vazio.txt"), "w") as f:
                pass
            # Remove pastas vazias
            for root, dirs, files in os.walk(windows_temp_path, topdown=False):
                for dir_name in dirs:
                    try:
                        ...
                        pass
            # Remove arquivo vazio.txt
            os.remove(os.path.join(windows_temp_path, "vazio.txt"))

    def delete_log_files(self):
        """Deleta arquivos de log do Windows."""
        log_paths = [
            os.path.join(os.environ["SystemRoot"], "Logs", "cbs"),
            os.path.join(os.environ["SystemRoot"], "Logs", "measuredboot"),
            os.path.join(os.environ["SystemRoot"], "Logs", "MoSetup"),
            os.path.join(os.environ["SystemRoot"], "Panther"),
            os.path.join(os.environ["SystemRoot"], "Performance", "WinSAT"),
            os.path.join(os.environ["SystemRoot"], "inf"),
            os.path.join(os.environ["SystemRoot"], "SoftwareDistribution"),
            os.path.join(os.environ["SystemRoot"], "Microsoft.NET"),
        ]
    ...
    def delete_vmware_logs(self):
        ...
        vmware_path = os.path.join(os.environ["ProgramData"], "VMware", "logs")
